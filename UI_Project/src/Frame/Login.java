	...
	public Login() {
		addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				int res = JOptionPane.showConfirmDialog(Login.this, "Are you sure to closing window?", "Confirm closing", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
		...
		label_2.setIcon(new ImageIcon("D:\\eclipse JAVA\\workspaceJAVA\\UI_Project\\UI_Project\\src\\Frame\\key.png"));
		...
		btnSignIn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(user_text.getText().trim().length() < 1 || new String(pass_text.getPassword()).trim().length() < 1) {
					...
					boolean check = UserManager.CheckLogin(user_text.getText().trim(),new String(pass_text.getPassword()).trim()); 
					if(check) {
		...
		btnSignIn.setBounds(151, 258, 94, 31);
		...
		btnSignUp.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(Limit_Register_window < 1) {
					...
					rs.setIconImage(icons);
					rs.setLocation(getX()+398, 0);
					rs.setVisible(true);
					rs.noResize();
				}
		...
		btnSignUp.setBounds(255, 258, 94, 31);
