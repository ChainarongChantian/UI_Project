...
public class UserManager {
	...
	public static boolean FindUser(String Newuser) {
		FindIterable<Document> iterateDoc = DBCollection.find();
		Iterator<Document> iterator = iterateDoc.iterator();
		while(iterator.hasNext()) {
	...
	public static boolean CheckLogin(String Newuser, String pass) {
		FindIterable<Document> iterateDoc = DBCollection.find();
		Iterator<Document> iterator = iterateDoc.iterator();
		while(iterator.hasNext()) {
			Document ds = iterator.next();
			if(ds.getString("username").trim().equals(Newuser) && 
			    ds.getString("password").trim().equals(pass)) {
				return false;
			}
		}
		return true;
